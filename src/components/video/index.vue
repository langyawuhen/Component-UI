      ...
      <el-header height="46px" class="video-container-header">
        <div class="header-box">
          <div class="header-box-item" v-for="(module,i) in moduleItems" :key="i">
            <el-icon class="pointer" size="25">
              <component :is="module.icon"/>
            </el-icon>
            <span class="pointer" v-if="module.isCloseAll">{{ module.currentValue }}</span>
            <el-dropdown v-else @command="handleCommand" popper-class="dropdown-showcase">
                        <span class="dropdown-showcase">
...
import CloseAll from "@/components/svgIcon/close-all.vue";
import CodeStream from "@/components/svgIcon/code-stream.vue";
import Layout from "@/components/svgIcon/layout.vue";
import Ratio from "@/components/svgIcon/ratio.vue";
